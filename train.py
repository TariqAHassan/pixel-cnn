...
args_dict = {
    # Data I/O ---
    'data_dir': '/tmp/pxpp/data',           # Location of the dataset
    'save_dir': '/tmp/pxpp/save',           # Location for parameter checkpoints and samples
    'data_set': 'cifar',                    # Can be either cifar|imagenet
    'save_interval': 20,                    # Every how many epochs to write checkpoint/samples.
    'load_params': False,                   # Whether or not to restore training from previous model checkpoint.
    ...
    'resnet_nonlinearity': 'concat_elu',    # Nonlinearity to use in the ResNet layers; 'concat_elu', 'elu' or 'relu'.
    'class_conditional': False,             # Condition generative model on labels?

    # Optimization ---
    'learning_rate': 0.001,                 # Base learning rate
    'lr_decay': 0.999995,                   # Learning rate decay, applied every step of the optimization
    'batch_size': 12,                       # Batch size during training per GPU
    'init_batch_size': 100,                 # How much data to use for data-dependent initialization
    'dropout_p': 0.5,                       # Dropout strength (1 - keep probability)
    'max_epochs': 5000,                     # Number of epochs to run in total.
    'nr_gpu': 1,                            # Number of GPUs

    # Evaluation ---
    'polyak_decay': 0.9995,                 # Exponential decay rate of the sum of previous model iterates during
                                            # Polyak averaging
...
# Parse ---
class ArgsDictWrapper(object):
    def __init__(self, args_dict):
        self.__dict__.update(args_dict)

args = ArgsDictWrapper(args_dict)
